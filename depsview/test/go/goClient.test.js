...
describe("escapeModulePath", () => {
  it("encodes uppercase letters as !lowercase per the GOPROXY protocol", () => {
    assert.equal(escapeModulePath("github.com/BurntSushi/toml"), "github.com/!burnt!sushi/toml");
  });

  it("leaves all-lowercase paths unchanged", () => {
    assert.equal(escapeModulePath("github.com/gin-gonic/gin"), "github.com/gin-gonic/gin");
  });
  ...
  it("preserves dots, slashes, hyphens, and digits", () => {
    assert.equal(escapeModulePath("gopkg.in/yaml.v3"), "gopkg.in/yaml.v3");
  });
});
...
describe("fetchModuleMod", () => {
  ...
  it("appends .mod to the version in the URL", async () => {
    ...
    assert.ok(
      capturedUrl.endsWith("v2.3.4.mod"),
      `expected URL to end with v2.3.4.mod, got: ${capturedUrl}`,
    );
  });
