...
describe("parseRequirementsTxt — security: path traversal via -r include", () => {
  ...
  test("does not read a file outside the project root via ../ traversal", () => {
    ...
    fs.writeFileSync(path.join(projectDir, "requirements.txt"), "-r ../outside.txt\nrequests\n");
    const { deps } = parseDependencyFile(projectDir);
    assert.ok(
      deps.some((d) => d.name === "requests"),
      "in-project package should be present",
    );
    assert.ok(
      !deps.some((d) => d.name === "evil-package"),
      "out-of-project package must not appear",
    );
  });
  ...
  test("does not read an absolute path via -r include", () => {
    ...
    assert.ok(
      !deps.some((d) => d.name === "evil-absolute"),
      "absolute-path package must not appear",
    );
  });
});
...
describe("parseRequirementsTxt — security: circular -r includes", () => {
  ...
  test("does not hang or crash on a self-referencing -r include", () => {
    fs.writeFileSync(path.join(tmpDir, "requirements.txt"), "-r requirements.txt\nrequests\n");
...
describe("parseRequirementsTxt — test -r include filtering", () => {
  ...
  test("includes -r requirements-test.txt when includeTests is true", () => {
    ...
    assert.ok(
      deps.some((d) => d.name === "pytest"),
      "test dep should be present with includeTests",
    );
  });
...
`;
  ...
  test("includes dev-dependencies when includeTests is true", () => {
    ...
    assert.ok(
      deps.some((d) => d.name === "pytest"),
      "pytest should be present with includeTests",
    );
    assert.ok(
      deps.some((d) => d.name === "black"),
      "black should be present with includeTests",
    );
  });
  ...
  test("still includes prod deps when includeTests is true", () => {
    ...
    assert.ok(
      deps.some((d) => d.name === "requests"),
      "requests should always be present",
    );
  });
});
...
`;
  ...
  test("includes group dependencies when includeTests is true", () => {
    ...
    assert.ok(
      deps.some((d) => d.name === "ruff"),
      "ruff should be present with includeTests",
    );
  });
});
...
`;
  ...
  test("still includes [packages] entries when includeTests is true", () => {
    ...
    assert.ok(
      deps.some((d) => d.name === "click"),
      "click should always be present",
    );
  });
...
describe("parseDependencyFile — error handling", () => {
  ...
  test("throws when no dependency file is found", () => {
    ...
    assert.throws(() => parseDependencyFile(__dirname), /No dependency file found/);
  });

  test("error message lists requirements_all.txt among the searched files", () => {
    assert.throws(() => parseDependencyFile(__dirname), /requirements_all\.txt/);
  });
});
