...
import { fetchDownloadCounts, getDownloadCount, _clearCache } from '../../src/npm/npmStatsClient.js';
...
afterEach(() => { globalThis.fetch = origFetch; _clearCache(); });
...
function setFetch(fn) {
  globalThis.fetch = async (url, opts) => { calls.push(url); return fn(url, opts); };
}
...
describe('fetchDownloadCounts', () => {
  ...
  it('returns null for a name whose request fails on the network', async () => {
    setFetch(() => { throw new Error('offline'); });
    const result = await fetchDownloadCounts(['ghost']);
    assert.equal(result.get('ghost'), null);
  });
