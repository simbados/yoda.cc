...
import {
  ...
  satisfiesRange,
  resolveVersion,
} from "../../src/npm/versionResolver.js";
...
describe("compareSemver", () => {
  ...
  it("2.0.0 > 1.0.0", () =>
    assert.ok(compareSemver(parseSemver("2.0.0"), parseSemver("1.0.0")) > 0));
  it("1.0.0 == 1.0.0", () =>
  ...
  it("1.0.0-1 < 1.0.0-alpha (numeric < string)", () =>
...
describe("satisfiesRange — spaced comparators", () => {
  ...
  it("3.0.0 does not satisfy >= 3.1.0 < 4", () =>
    assert.ok(!satisfiesRange("3.0.0", ">= 3.1.0 < 4")));
  it("4.0.0 does not satisfy >= 3.1.0 < 4", () =>
    assert.ok(!satisfiesRange("4.0.0", ">= 3.1.0 < 4")));
...
describe("satisfiesRange — multiple spaces between operator and version", () => {
  ...
  it("3.3.1 satisfies >=   3.1.0    <   4", () =>
    assert.ok(satisfiesRange("3.3.1", ">=   3.1.0    <   4")));
  it("4.0.0 does not satisfy >=   3.1.0    <   4", () =>
    assert.ok(!satisfiesRange("4.0.0", ">=   3.1.0    <   4")));
});
