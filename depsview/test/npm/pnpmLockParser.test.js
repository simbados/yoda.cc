...
describe('indexOfDepPathSuffix', () => {
  describe('no trailing paren', () => {
    it('returns -1 for both indices when input has no trailing )', () => {
      assert.deepEqual(
        indexOfDepPathSuffix('pkg@1.0.0'),
        { peersIndex: -1, patchHashIndex: -1 },
      );
    });
...
describe('stripPnpmSuffix', () => {
  ...
  describe('peer suffix', () => {
    ...
    it('removes multiple peer groups starting from the outermost', () => {
      assert.equal(
        stripPnpmSuffix('pkg@1.0.0(react@18.0.0)(react-dom@18.0.0)'),
        'pkg@1.0.0',
      );
    });
  ...
  describe('patch suffix', () => {
    ...
    it('removes both patch and peer when both present (patch wins)', () => {
      assert.equal(
        stripPnpmSuffix('pkg@1.0.0(patch_hash=abc)(react@18.0.0)'),
        'pkg@1.0.0',
      );
    });
  });
...
describe('extractResolutionType', () => {
  describe('implicit tarball form', () => {
    it('returns null for a typical TarballResolution line', () => {
      assert.equal(
        extractResolutionType('resolution: {integrity: sha512-aaa, tarball: https://example.invalid/x.tgz}'),
        null,
      );
    });
  ...
  describe('git', () => {
    it('returns git for a git resolution', () => {
      assert.equal(
        extractResolutionType("resolution: {commit: abc, repo: 'git+ssh://git@github.com/foo/bar.git', type: git}"),
        'git',
      );
    });
  });
  ...
  describe('custom resolver', () => {
    it('returns the full custom:foo token', () => {
      assert.equal(
        extractResolutionType('resolution: {type: custom:foo}'),
        'custom:foo',
      );
    });
  });
...
describe('parsePackageKey', () => {
  describe('v5', () => {
    it('splits a plain key on the last slash', () => {
      assert.deepEqual(
        parsePackageKey('/lodash/4.17.21', 5),
    ...
    it('handles scoped names using lastIndexOf(/)', () => {
      assert.deepEqual(
        parsePackageKey('/@babel/core/7.24.0', 5),
  ...
  describe('v6', () => {
    it('splits on indexOf(@,1) so scoped names parse correctly', () => {
      assert.deepEqual(
        parsePackageKey('/@babel/core@7.24.0', 6),
        { name: '@babel/core', version: '7.24.0' },
      );
    });
    ...
    it('strips a single peer suffix', () => {
      assert.deepEqual(
        parsePackageKey('/eslint@8.57.0(typescript@5.0.0)', 6),
    ...
    it('strips multiple peer suffixes using the outermost-paren walk', () => {
      assert.deepEqual(
        parsePackageKey('/eslint@8.57.0(typescript@5.0.0)(react@18.0.0)', 6),
        { name: 'eslint', version: '8.57.0' },
      );
    });
  });
  ...
  describe('v9', () => {
    it('parses a plain key', () => {
      assert.deepEqual(
        parsePackageKey('lodash@4.17.21', 9),
        { name: 'lodash', version: '4.17.21' },
      );
    });
    ...
    it('parses a scoped name', () => {
      assert.deepEqual(
        parsePackageKey('@babel/core@7.24.0', 9),
        { name: '@babel/core', version: '7.24.0' },
      );
    });
    ...
    it('strips both patch and peer suffixes', () => {
      assert.deepEqual(
        parsePackageKey('foo@1.0.0(patch_hash=abc)(react@18.0.0)', 9),
    ...
    it('preserves an npm: alias version with the embedded @', () => {
      assert.deepEqual(
        parsePackageKey('react-alt@npm:react@18.2.0', 9),
        { name: 'react-alt', version: 'npm:react@18.2.0' },
      );
    });

    it('preserves a git+ssh URL with an embedded @', () => {
      assert.deepEqual(
        parsePackageKey('pkg@git+ssh://git@github.com/foo/bar.git#abc', 9),
        { name: 'pkg', version: 'git+ssh://git@github.com/foo/bar.git#abc' },
      );
    });
