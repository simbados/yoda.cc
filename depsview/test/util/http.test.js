...
describe("fetchWithRetry — success", () => {
  it("returns parsed JSON on 200", async () => {
    mockFetch({
      ok: true,
      status: 200,
      json: async () => ({ name: "lodash" }),
      headers: { get: () => null },
    });
    const result = await fetchWithRetry("https://pypi.org/pypi/lodash/json", {
      serviceName: "test",
    });
  ...
  it("forwards custom headers to fetch", async () => {
    ...
    await fetchWithRetry("https://registry.npmjs.org/lodash", {
  ...
  it("returns text when responseType is text", async () => {
    ...
    const result = await fetchWithRetry("https://registry.npmjs.org/lodash", {
      responseType: "text",
    });
...
describe("fetchWithRetry — 404", () => {
  it("returns null regardless of throwOnError", async () => {
    for (const throwOnError of [true, false]) {
      ...
      const result = await fetchWithRetry("https://pypi.org/pypi/nonexistent/json", {
        serviceName: "test",
        throwOnError,
      });
...
describe("fetchWithRetry — non-ok response (throwOnError=true)", () => {
  it("throws on 500", async () => {
    ...
    await assert.rejects(
      () =>
        fetchWithRetry("https://pypi.org/pypi/pkg/json", {
...
describe("fetchWithRetry — non-ok response (throwOnError=false)", () => {
  it("returns null on 500", async () => {
    mockFetch({ ok: false, status: 500, headers: { get: () => null } });
    const result = await fetchWithRetry("https://pypi.org/pypi/pkg/json", {
      serviceName: "test",
      throwOnError: false,
    });
    assert.equal(result, null);
