...
import { domainOf, groupByDomain } from "../../src/output/nonStandardSources.js";
...
describe("domainOf", () => {
  describe("valid URL strings", () => {
    ...
    it("returns the hostname from a URL with a path and query string", () => {
      assert.equal(
        domainOf("https://npm.pkg.github.com/@myorg/mypkg/-/mypkg-1.0.0.tgz"),
        "npm.pkg.github.com",
      );
    });
    ...
    it("returns the hostname from a URL with a port", () => {
      assert.equal(
        domainOf("https://registry.corp.invalid:4873/pkg/-/pkg-2.0.0.tgz"),
        "registry.corp.invalid",
      );
    });
  });
  ...
  describe("non-URL strings (Go module path fallback)", () => {
    ...
    it("returns the first path segment for a multi-segment Go path", () => {
      assert.equal(
        domainOf("private.registry.example.invalid/group/project"),
        "private.registry.example.invalid",
      );
    });
...
describe("groupByDomain", () => {
  ...
  it("handles Go module paths (non-URL) by using the first path segment as domain", () => {
    const result = groupByDomain([{ name: "corp.invalid/mylib", url: "corp.invalid/mylib" }]);
    assert.equal(result.size, 1);
