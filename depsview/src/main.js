...
function parseArgs() {
  ...
  const socketKey = socketKeyArg
    ? socketKeyArg.slice("--socket-key=".length)
    : (process.env.SOCKET_KEY ?? null);
  const socketOrg = socketOrgArg
    ? socketOrgArg.slice("--socket-org=".length)
    : (process.env.SOCKET_ORG ?? null);
  ...
  if (!packageName && positional.length === 0) {
    ...
    console.error("");
    console.error(
      "Ecosystem flags act as filters; with none, every detected ecosystem is included.",
    );
    console.error(
      "--package / -p: resolve a single package by name (requires exactly one ecosystem flag).",
    );
...
async function confirmWarning(warning) {
  ...
  if (!process.stdin.isTTY) {
    process.stderr.write(
      "Non-interactive session detected — aborting. Re-run in a terminal to confirm.\n\n",
    );
    return false;
  }
...
async function main() {
  ...
  if (packageName) {
    if (requestedEcosystems.size !== 1) {
      console.error(
        "Error: --package / -p requires exactly one ecosystem flag (--npm, --python, or --go).",
      );
      process.exit(1);
    }
  ...
  const sections = await orchestrate(ctx, {
    ecosystems,
    includeTests,
    downloadStats,
  ...
  if (
    !anyResults &&
    [...sections.values()].every((s) => s.error || !s.results || s.results.size === 0)
  ) {
